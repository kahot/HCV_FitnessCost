...
#remove the ns factors:  NS5A and Shape        
...
#Calculate the effect sizes and save the best model
effect<-BetaEffectSize(mod2)
write.csv(effect, "Output/BetaReg/BetaReg_MF_effectsize.csv", row.names = F)


#Comparison for SC (save for later)
mod4 <- update(mod1, ~.-NS4A -NS3 -Shape)

effect2<-BetaEffectSize(mod4)
write.csv(effect2, "Output/BetaReg/BetaReg_MF_effectsize_SCmodel.csv", row.names = F)
