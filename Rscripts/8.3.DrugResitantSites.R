...
HCVFiles<-list.files("Output/Overview/", pattern="overview.csv")
...
for (i in 1:length(HCVFiles)){ 
        df<-read.csv(paste0("Output/Overview/",HCVFiles[i]),stringsAsFactors=FALSE, row.names = 1)
...
for (i in 1:length(HCVFiles3)){ 
        df<-read.csv(paste0("Output/OverviewF/",HCVFiles3[i]),stringsAsFactors=FALSE, row.names = 1)
...
### Create a figure ###
#dr_sc<-read.csv("Output/DrugRes/RAV.SC_summary.csv")

pdf(paste0("Output/DrugRes/RAVs.sc.pdf"), height = 10.5, width = 14.5)
...
#Fig 6
pdf(paste0("Output/DrugRes/RAVs.SCs.pdf"), height = 9, width = 15.5)
layout(matrix(c(1,2), byrow = TRUE), heights=c(1,3))
par(mar=c(0, 5, 2, 5))
genesDF<-data.frame("name"= c("NS3","NS5A","NS5B"), "Begin"= c(1,ns3n+1,(ns3n+ns5an+1)),"End"= c(ns3n,(ns3n+ns5an),nrow(DR_mutfreq)))
...
for (i in 1:3){
    ...
    abline(v=v1, col=paste0(colors[i],"66"),lty=1, lwd=15)
    abline(v=v2, col=paste0(colors[i],"1A"),lty=1, lwd=15)
}
...
#######
#Fig. S7
DF<-merge(DR_diff[,c("ID","Percent")],NatPrev[,c("ID","Percent")], by="ID")
colnames(DF)[2:3]<-c("% Fixed samples","% Observed")

DF$ID<-factor(DF$ID, levels=paste(DR_diff$ID))
DF<-DF[order(DF$ID),]
DF$Gene<-c(rep("NS3", times=42), rep("NS5A", times=36), rep("NS5B", times=9))

DF$Type<-DR$Type
#set color
c <- ifelse(DF$Type == "Ts","black", "orange4")

ggplot(DF, aes(x=ID, y=Fixed, fill=Gene))+
    geom_bar(stat="identity")+
    ylab("% Fixed samples")+xlab('')+
    theme_classic()+
    scale_fill_manual(values=colors[1:3])+
    theme(legend.title = element_blank(), axis.text.x = element_text(angle=90, hjust=1, size=5))

DFm<-melt(DF)
DFm$variable<-factor(DFm$variable, levels=c("% Observed","% Fixed samples"))

ggplot(DFm, aes(x=ID, y=value, fill=Gene))+
    geom_bar(stat="identity")+
    geom_vline(xintercept =c(42.5,78.5) ,col='gray50', lwd=0.6)+
    facet_wrap(~variable, ncol=1)+
    ylab("Percent")+xlab('')+
    theme_bw()+
    scale_fill_manual(values=colors[1:3])+
    theme(panel.grid.major.x = element_blank(),panel.grid.minor = element_blank(),
          legend.title = element_blank(), axis.text.x = element_text(angle=90, hjust=1, size=6.5, color=c))
ggsave("Output/DrugRes/Observed_fixed_percent_sample.pdf", width = 9, height = 6)
