...
for (i in 1:length(HCVFiles_overview3)){ 
        overviews2<-read.csv(paste0("Output/Overview3/",HCVFiles_overview3[i]),stringsAsFactors=FALSE, row.names=1)
...
for (i in 1:5) {
        ...
        write.csv(dat2,paste0("Output/MutFreq.filtered/Filtered.",files[i],".Q35.csv"))
}
...
SeqDt<-list.files("Output/SeqDataQ35/",pattern="SeqData")
...
for (i in 1:length(SeqDt)){   
        ...
        print(id)
        DF<-read.csv(paste0("Output/SeqDataQ35/",SeqDt[i]),stringsAsFactors=FALSE, row.names = 1)
...
write.csv(Reads_Ts, "Output/ReadDepth_Transitions.csv")

## Average Reads at each position (n)
Reads<-list()
for (i in 1:length(SeqDt)){   
        #for (i in 1:1){
        id<-substr(paste(SeqDt[i]),start=9,stop=15)
...
write.csv(Reads_Total, "Output/ReadDepth_All.csv")

Reads_Total$Depth<-rowSums(Reads_Total[2:196], na.rm=T)
Reads_Total$Mean.Depth<-rowMeans(Reads_Total[2:196], na.rm=T)

# Use general read deapth for now:
depth<-Reads_Total[,c("pos","Depth")]
write.csv(depth,"Output/ReadDepth_sum.csv")
...
write.csv(TB3, "Output/MutFreq.filtered/MF.Mean.SE.summary_updated.csv")
...
ggsave("Output/SummaryFig.Filtered/MutFreq.byGene.byType.pdf", width=8, height = 4)       
...
write.csv(Ts2,"Output/MutFreq.filtered/Ts_C_summary.csv")
